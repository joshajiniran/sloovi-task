...
def test_get_single_template(client, token, create_template):
    ...
    resp = client.get(f"/template/{temp['_id']}", headers={"Authorization": f"Bearer {token}"})
    assert resp.status_code == 200
...
def test_get_single_template_invalid_id(client, token):
    resp = client.get("/template/62978a4d5e41441d2cdcbf38", headers={"Authorization": f"Bearer {token}"})
